...
class TestGatewayPidState:
    ...
    def test_get_running_pid_removes_stale_default_pid_file(self, tmp_path, monkeypatch):
        """Regression: stale default gateway.pid must be unlinked during cleanup.

        Before this fix, get_running_pid() returned None for a stale record but
        left ~/.hermes/gateway.pid in place because cleanup delegated to
        remove_pid_file(), which refuses to delete a file owned by another PID.
        The next startup then hit write_pid_file() -> FileExistsError and the
        gateway flapped forever.
        """
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        pid_path = tmp_path / "gateway.pid"
        pid_path.write_text(json.dumps({
        ...
        monkeypatch.setattr(status.os, "kill", _missing_process)
