...
class _LoopBoundSession:
    def __init__(self, loop, *, closed: bool = False):
        self._loop = loop
        self.closed = closed
...
class TestWeixinReconnectHandling:
    @pytest.mark.asyncio
    async def test_poll_loop_session_expired_promotes_retryable_fatal(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        adapter = _make_adapter()
        poll_session = _AsyncClosable()
        send_session = _AsyncClosable()
        adapter._poll_session = poll_session
        adapter._send_session = send_session
        adapter._running = True

        async def _disconnecting_handler(_adapter):
            await adapter.disconnect()

        fatal_handler = AsyncMock(side_effect=_disconnecting_handler)
        adapter.set_fatal_error_handler(fatal_handler)

        with patch(
            "gateway.platforms.weixin._get_updates",
            new=AsyncMock(return_value={"ret": weixin.SESSION_EXPIRED_ERRCODE, "errcode": 0}),
    ...
    @pytest.mark.asyncio
    async def test_poll_loop_consecutive_errors_trigger_retryable_fatal(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        adapter = _make_adapter()
        adapter._poll_session = _AsyncClosable()
        adapter._running = True
        ...
        with patch(
            ...
            new=AsyncMock(
                side_effect=[
                    ...
                    RuntimeError("network still down"),
                    RuntimeError("network still down"),
                ]
            ),
        ), patch("gateway.platforms.weixin.asyncio.sleep", new_callable=AsyncMock) as sleep_mock:
        ...
        assert adapter.has_fatal_error
        assert adapter.fatal_error_code == "weixin_poll_error"
        assert adapter.fatal_error_retryable is True
        fatal_handler.assert_awaited_once()
        assert sleep_mock.await_count == weixin.MAX_CONSECUTIVE_FAILURES - 1


class TestWeixinDirectSendLoopSafety:
    @pytest.mark.asyncio
    async def test_direct_send_reuses_live_adapter_on_same_loop(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        current_loop = asyncio.get_running_loop()
        live_session = _LoopBoundSession(current_loop)
        live_adapter = MagicMock()
        live_adapter._send_session = live_session
        live_adapter._running = True
        live_adapter.format_message.side_effect = lambda content: content
        live_adapter.send = AsyncMock(return_value=weixin.SendResult(success=True, message_id="live-msg"))
        monkeypatch.setitem(weixin._LIVE_ADAPTERS, "live-token", live_adapter)

        session_cm = MagicMock()
        session_cm.__aenter__ = AsyncMock()
        session_cm.__aexit__ = AsyncMock(return_value=None)

        with patch("gateway.platforms.weixin.aiohttp.ClientSession", return_value=session_cm) as client_session_mock:
        ...
        live_adapter.send.assert_awaited_once_with("wxid_test123", "hello from same loop")
        client_session_mock.assert_not_called()

    @pytest.mark.asyncio
    async def test_direct_send_avoids_live_adapter_on_different_loop(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HERMES_HOME", str(tmp_path))
        foreign_loop = asyncio.new_event_loop()
        live_session = _LoopBoundSession(foreign_loop)
        live_adapter = MagicMock()
        live_adapter._send_session = live_session
        live_adapter._running = True
        live_adapter.format_message.side_effect = lambda content: content
        ...
        fallback_session.closed = False
        session_cm = MagicMock()
        session_cm.__aenter__ = AsyncMock(return_value=fallback_session)
        session_cm.__aexit__ = AsyncMock(return_value=None)

        send_mock = AsyncMock(return_value=weixin.SendResult(success=True, message_id="fresh-msg"))
        try:
            with patch("gateway.platforms.weixin.aiohttp.ClientSession", return_value=session_cm) as client_session_mock, \
                 patch.object(weixin.WeixinAdapter, "send", send_mock):
                result = await weixin.send_weixin_direct(
                    extra={"account_id": "acct"},
                    token="live-token",
                    chat_id="wxid_test123",
                    message="hello from other loop",
                )
        finally:
            foreign_loop.close()
        ...
        send_mock.assert_awaited_once_with("wxid_test123", "hello from other loop")
