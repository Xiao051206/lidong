...
class WeixinAdapter(BasePlatformAdapter):
    ...
    async def disconnect(self) -> None:
        ...
        if poll_task and not poll_task.done():
            if poll_task is not current_task:
                poll_task.cancel()
                try:
                    await poll_task
    ...
    async def _raise_retryable_poll_failure(self, code: str, message: str) -> None:
        """Promote a stuck Weixin poll loop into gateway-level reconnect handling."""
        ...
        try:
            ...
            logger.exception("[%s] Fatal Weixin poll notification failed", self.name)
            ...
            try:
                ...
                if ret not in (0, None) or errcode not in (0, None):
                    if ret == SESSION_EXPIRED_ERRCODE or errcode == SESSION_EXPIRED_ERRCODE:
                        await self._raise_retryable_poll_failure(
                            "weixin_session_expired",
                            "Weixin polling session expired; restarting adapter to refresh iLink state.",
                        )
                        return
                    ...
                    if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                        await self._raise_retryable_poll_failure(
                            ...
                            (
                                ...
                                f"(ret={ret}, errcode={errcode}, errmsg={errmsg or 'unknown'}). "
                                "Restarting adapter."
                            ),
                        )
                        return
                ...
                if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                    await self._raise_retryable_poll_failure(
                        "weixin_poll_error",
                        (
                            f"Weixin polling failed {consecutive_failures} consecutive times. "
                            f"Last error: {exc}"
                        ),
                    )
                    return
    ...
    async def send(
        ...
        try:
            ...
            logger.info(
                "[%s] send succeeded to=%s chunks=%d message_id=%s",
                self.name,
                _safe_id(chat_id),
                len(chunks),
                last_message_id,
...
async def send_weixin_direct(
    *,
    ...
    can_reuse_live_adapter = (
        ...
        and getattr(live_adapter, "_running", True)
        and session_loop is current_loop
        and not session_loop.is_closed()
    )
    if live_adapter is not None and send_session is not None and not send_session.closed and not can_reuse_live_adapter:
        logger.debug(
            "weixin direct send: live adapter session loop mismatch or adapter not running "
            "(current_loop=%r, session_loop=%r, adapter_running=%r); using one-shot session",
            current_loop,
            session_loop,
            getattr(live_adapter, "_running", None),
        )
    if can_reuse_live_adapter:
        ...
        logger.info(
            "weixin direct send succeeded via live adapter to=%s message_id=%s media_count=%d",
            _safe_id(chat_id),
            last_result.message_id if last_result else None,
            len(media_files or []),
        )
        return {
            "success": True,
    ...
    async with aiohttp.ClientSession(trust_env=True, connector=_make_ssl_connector()) as session:
        ...
        logger.info(
            "weixin direct send succeeded via one-shot session to=%s message_id=%s media_count=%d",
            _safe_id(chat_id),
            last_result.message_id if last_result else None,
            len(media_files or []),
        )
